...
describe('Grid', () => {
  ...
  it('should initialize with correct width and height', () => {
    const id = 1;
    const grid = new Grid({ id, size: { width: 10, height: 5 }, basePath });
  ...
  it('should create rows based on height', () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect(Row).toHaveBeenCalledWith({ id: i, length: 10, basePath });
    }
  });
  ...
  it('should render the grid correctly', () => {
    ...
    const grid = new Grid({ id, size: { width: 3, height: 2 }, basePath });
